...
def make_back_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
	...
	bookname = make_prettier(bookname)
	...
	if cover_style == "Type 1":
		...
		blurb = make_prettier(blurb)

		c.setFillColorRGB(0.1, 0.1, 0.1)

		newline2 = make_prettier(bookname)
		...
		while ypos > 192:
			...
			thisblurb = make_prettier(thisblurb)
			if DEBUG == 1:
				print "========================="
		...
		make_back_cover(c=c, VERBOSE=VERBOSE, outfileName=outfileName, width=width, height=height, author=author,
						county=county, towns=towns, bookname=bookname, blurb=blurb,
						background_colour=background_colour, foreground_colour=foreground_colour,
						ISBN_text=ISBN_text, ISBN_for_barcode=ISBN_for_barcode, price=price,
...
def make_front_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
	...
	newline2 = make_prettier(newline2)
...
##                      height=illustration.size[1]+bleed)
	...
	if cover_style == "Type 1":
		...
		newline3 = make_prettier(newline3)
...
def make_text_front_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
						  blurb, background_colour=None, foreground_colour=None):
	#DEBUG = 1
	DEBUG = 0
	...
	if os.path.isfile(os.path.join(outdir, "%s.txt" % county)):
		if os.path.isfile(os.path.join(outdir, "%s_OLD.txt" % county)):
			new_fname = "%s_OLD_%s.txt" % (county, "%06d" % random.choice(range(0,99999)))
		else:
			new_fname = "%s_OLD.txt" % county
		os.rename(os.path.join(outdir, "%s.txt" % county),
				  os.path.join(outdir, new_fname))

	outfile = open(os.path.join(outdir, "%s.txt" % county), "w")
...
def make_text_back_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
						 blurb, background_colour=None, foreground_colour=None,
	...
	if DEBUG == 1:
		...
		outdir = os.path.join(startdir, "OUTPUT")
	outfile = open(os.path.join(outdir, "%s.txt" % county), "a")
	...
	if VERBOSE > 1:
		...
		print back_cover_text
	write_page_to_disk(outfileName=os.path.join(outdir, "%s.txt" % county),
	...
	make_text_back_cover_final_twiddles(VERBOSE=1, outfileName="%s.txt" % county,
										price="£4.99", ISBN="0 9999999999999",
										pagewidth=80)
...
def pick_cover_pict():
	...
	#if we were really clever, we might make sure we use one that's actually used in the book...

	this_dir= os.getcwd()

	if os.path.isdir(os.path.join("images", "old_book_illustrations", "buildings")):
		buildings_dir = os.path.join("images", "old_book_illustrations", "buildings")
	elif os.path.isdir(os.path.join("..", "images", "old_book_illustrations", "buildings")):
		buildings_dir = os.path.join("..", "images", "old_book_illustrations", "buildings")

	os.chdir(buildings_dir)
	buildingpics = glob.glob("*.jpg")

	#pics that have shown up in testing and aren't really suitable for front cover use...
	exceptions = ["copenhagen-thorvaldsen-museum-1200.jpg",
				  "york-elevation-1200.jpg",
				  "turner-house.jpg"]

	for p in exceptions:
		buildingpics.remove(p)

	pic_to_use = random.choice(buildingpics)
	pic_to_use = os.path.join(buildings_dir, pic_to_use)
...
def getCovers(c, VERBOSE, outfileName, width, height, author, county, towns, bookname, blurb):
	...
	if VERBOSE == 1:
		print "\tcreating front cover..."
	c = make_front_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
						 blurb, background_colour=back, foreground_colour=fore,
						 cover_style=cover_style)

	make_text_front_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
						  blurb, background_colour=None, foreground_colour=None)
	...
	c, blurb = make_back_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
							   blurb, background_colour=back, foreground_colour=fore,
							   ISBN_text=ISBN, ISBN_for_barcode=ISBN_for_barcode, price=price,
							   cover_style=cover_style)

	make_text_back_cover(c, VERBOSE, outfileName, width, height, author, county, towns, bookname,
...
def run(c, VERBOSE, outfileName, width, height, author, county, towns, bookname, blurb, cover_style=None):
...
if __name__ == "__main__":
	...
	for s in POSS_COVER_STYLES:
		...
		run(c, VERBOSE, outfileName, width, height, author, county, towns, bookname, blurb, cover_style=s)
